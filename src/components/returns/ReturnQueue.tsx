...
export function ReturnQueue({ sourceType, filter, onSelectReturn, emptyMessage, showDecision = true }: Props) {
  ...
  const { data, loading, error, refresh } = useReturns({ limit: 100, ...(sourceType ? { source_type: sourceType } : {}) });
  ...
  return <section className="returns-panel queue-panel">
    ...
    {loading ? <LoadingState message="Loading returns…"/> : error ? <ErrorState title="Unable to load returns" message={error} onRetry={refresh}/> : rows.length === 0 ? <div className="empty-panel">{emptyMessage}</div> : <div className="table-scroll"><table className="returns-table investigation-table"><thead><tr><th>Return</th><th>Product / Customer</th><th>Workflow stage</th><th>Previous risk</th><th>Current risk</th>{showDecision && <th>Decision</th>}<th>Last updated</th><th></th></tr></thead><tbody>{rows.map((item) => <tr key={item.return_id} onClick={() => onSelectReturn(item.return_id)}><td><strong>{item.return_id}</strong><SourceBadge sourceType={item.source_type}/></td><td><div className="case-identity-cell"><span className="case-product-icon"><Package size={16}/></span><div><strong>{item.product.name}</strong><small><UserRound size={11}/>{item.customer.display_name}</small></div></div></td><td><span className={'workflow-badge workflow-' + (item.status.toLowerCase().includes('rejected') ? 'rejected' : item.status.toLowerCase().includes('approved') ? 'approved' : item.status.toLowerCase().includes('inspection') ? 'inspection' : item.status.toLowerCase().includes('review') ? 'review' : 'neutral')}>{item.status}</span></td><td>{item.previous_risk ? <RiskBadge score={item.previous_risk.score} band={item.previous_risk.band} size="sm"/> : <span className="muted-value">—</span>}</td><td>{item.risk ? <RiskBadge score={item.risk.score} band={item.risk.band}/> : <span className="neutral-pill">NOT ASSESSED</span>}</td>{showDecision && <td><DecisionBadge value={item.decision}/></td>}<td className="date-cell">{formatDateTime(item.assessment_at || item.requested_at)}</td><td><ArrowRight className="row-arrow" size={17}/></td></tr>)}</tbody></table></div>}
