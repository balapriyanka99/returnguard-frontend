...
export function DashboardPage({ onNavigate, onSelectReturn }: { onNavigate: (path: string) => void; onSelectReturn: (id: string) => void }) {
  ...
  const attentionRows = recent.filter((item) => item.decision === 'MANUAL_REVIEW' || item.decision === 'REQUIRE_INSPECTION' || item.decision === 'REJECT_OR_ESCALATE' || item.risk?.band === 'high' || item.risk?.band === 'critical').slice(0, 5);
  ...
  return <div className="page-stack overview-page">
    ...
    <div className="overview-focus-grid">
      <section className="attention-panel"><div className="panel-heading"><div><span className="eyebrow">Priority queue</span><h2><BellRing size={17}/>Returns Needing Attention</h2></div><button className="rg-btn-secondary" onClick={() => onNavigate('/investigations')}>Open queue<ArrowRight size={15}/></button></div>
        {attentionRows.length ? attentionRows.map((item) => <button className={'attention-row attention-' + (item.risk?.band || 'neutral')} key={item.return_id} onClick={() => onSelectReturn(item.return_id)}><div className="attention-id"><strong>{item.return_id}</strong><SourceBadge sourceType={item.source_type}/></div><div className="attention-product"><strong>{item.product.name}</strong><small>{item.reason_for_attention || item.reason}</small></div><span className="workflow-badge">{item.status}</span><div className="attention-risk">{item.risk ? <RiskBadge score={item.risk.score} band={item.risk.band}/> : <span className="neutral-pill">Undetermined</span>}</div><span className={'decision-badge decision-' + (item.decision === 'REJECT_OR_ESCALATE' ? 'reject' : item.decision === 'MANUAL_REVIEW' ? 'review' : item.decision === 'REQUIRE_INSPECTION' ? 'inspection' : 'pending')}>{decisionLabel(item.decision)}</span><ArrowRight className="row-arrow" size={18}/></button>) : <div className="empty-panel">No returns currently need attention.</div>}
      </section>
