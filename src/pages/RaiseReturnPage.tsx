...
export function RaiseReturnPage({ onBack, onCreated }: { onBack: () => void; onCreated: (id: string) => void }) {
  ...
  return <div className="flow-page">
    ...
    <section className="wizard-card">
      ...
      {step === 4 && <><div className="section-heading"><Check/><div><h2>Review return request</h2><p>Only the selected order item, reason, and optional comment will be submitted.</p></div></div><div className="review-grid"><div><span>Customer</span><strong>{customer?.display_name}</strong><small>User ID {customer?.user_id}</small></div><div><span>Order</span><strong>#{order?.order_id}</strong><small>{order && formatDate(order.created_at)}</small></div><div><span>Product</span><strong>{item?.product_name}</strong><small>Order item {item?.order_item_id}</small></div><div><span>Return reason</span><strong>{reason}</strong><small>{comment || 'No comment provided'}</small></div></div><div className="neutral-note">Inspection, vision, risk, and decision data may be unavailable immediately after creation. This is expected and remains neutral.</div></>}
